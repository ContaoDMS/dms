		...
		protected function getCategoryTree ($parentId, $arrGroups, $category) {
			...
			while ($objDocumentManagementSystemKat->next()) {
				...
				(
					...
					'recht_mehralslesen'      => $mrkRechtMehrAlsLesen,
					'recht_verwalten'         => $mrkRechtVerwalten,
					'recht_upload'            => $mrkRechtUpload,
					'recht_editieren'         => $tmp_editieren,
					'recht_loeschen'          => $tmp_loeschen,
					'recht_veroeffentlichen'  => $tmp_veroeffentlichen,
					'category'  => $category,
				);	
				...
				*/
				...
				// Anzeige der Daten
					...
					(
						'kategorieid'             => $intKategorieId,
						'kategoriename'           => $strKategorieName,
						'recht_veroeffentlichen'  => $strVeroeffentlichen,
						'dateitypen'              => $strDateitypen,
					);
        ...
        protected function dokument_upload_eigenschaften($intKategorieId,$strKategorieName,$strVeroeffentlichen,$strDateiTypen,$strUploadName,$strUploadTyp,$strUploadGroesse,$strUploadVerzeichnis,$strUploadFehler)
				...
				//     Prüfen, ob das Dokument bereits in anderen Versionen existiert
						...
						else                                      // Dokument in DB vorhanden
						{
							...
							foreach ($rows as $row)
							{
								...
								$strDokName            = $row['name'];
								$strDokBeschreibung    = html_entity_decode($row['dk_beschreibung']);
								$intVersionMajor       = $row['version_major'];
								$intVersionMinir       = $row['version_minor']+1;
								$arrStichworte         = explode(",", html_entity_decode($row['dk_stichworte']));
								$ind++;						
							}
						}
				...
				// Anzeige der Daten
						...
						(
							...
							'kategoriename'           => $strKategorieName,        
							'dateitypen'              => $strDateiTypen,
							'datei_name'              => $strDateiNameUpload,
							'datei_typ'               => $strDateiTyp,
							'datei_groesse'           => $strUploadGroesse,
							'upload_verzeichnis'      => $strUploadVerzeichnis,
							'upload_fehler'           => $strUploadFehler,
							'upload_erlaubt'          => $mkUploadErlaubt,
							'version_major'           => $intVersionMajor,
							'version_minir'           => $intVersionMinir,
							'recht_veroeffentlichen'  => $strVeroeffentlichen,
							'versionvorhanden'        => $mkVersionVorhanden,
							'dokname'                 => $strDokName,
							'dokbeschreibung'         => $strDokBeschreibung,
							'stichworte'              => $arrStichworte,
							'rows'                    => $rows,
						);
				...
				*/	
				...
				else
				{
					...
					(
						...
						'name'              => $strDokName,
						'pid'       => $intKategorieId,
						'dk_beschreibung'      => $strDokBeschreibung,
						'file_source'             => $strDateiName,
						'file_sourcetyp'          => $strDateiTyp,
						'file_sourcegroesse'      => $strDateiGroesse,
						'version_major'     => $strDokVersionMajor,
						'version_minor'     => $strDokVersionMinir,
						'file_preview'              => $strBildNameUpload,
						'dk_stichworte'        => $strDokStichworte,
						'upload_member'     => $intUserId,
						'upload_date'      => $strDatumAktuell,
						'published'   => $intDokVeroeffentlichen,
					);
					$this->Database->prepare("INSERT INTO tl_dms_document %s")
								->set($set)
								->execute();
				...
				(
				  'kategoriename'           => $strKategorieName,
				  'datei_name'              => $strDateiName,
				  'datei_groesse'           => $strDateiGroesse,
				  'datei_typ'               => $strDateiTyp,
				  'dokname'                 => html_entity_decode($strDokName),
				  'dokbeschreibung'         => html_entity_decode($strDokBeschreibung),
				  'dokversionmajor'         => $strDokVersionMajor,
				  'dokversionminir'         => $strDokVersionMinir,
				  'dokveroeffentlichen'     => $intDokVeroeffentlichen,
				  'bildname'                => $strBildNameUpload,
				  'bildgroesse'             => $strBildGroesse,
				  'stichworte'              => html_entity_decode($strDokStichworte),
				  'bildfehlermeldung'       => $intBildFehlerMeldung,
				  'dateifehlermeldung'      => $intVersionFehlerMeldung,
				);
				...
				*/
				...
				// Anzeige der Daten
					...
					(
				  	...
				  	'recht_editieren'					=> $strEditieren,
				  	'recht_loeschen'					=> $strLoeschen,
				  	'recht_veroeffentlichen'	=> $strVeroeffentlichen,
					...
					*/
					...
					foreach ($arrLoeschen as $strLoeschenId)
					{
						...
						(
							...
							'recht_loeschen'					=> $strLoeschen,
							'recht_editieren'					=> $strEditieren,
							'arr_veroeffentlichen'		=> $arrConvVeroeffentlichen,
							'arr_loeschen'						=> $arrConvLoeschen,
							'arr_editieren'						=> $arrConvEditieren,
							'dokdetails'							=> $rows,
							'editierlauf'							=> $mkEditierLauf,
						);					
					}
					...
					*/
					...
					foreach ($arrLoeschen as $strLoeschenId)
					{
						...
						while ($objDocumentManagementSystemDok->next())
						{
							...
							$this->Database->prepare("DELETE FROM tl_dms_document WHERE id = $strLoeschenId")
										->execute();
						}
												// Anzeige der Daten
						$arrDocumentManagementSystemVerw[] = array
						(
							...
							'arr_loeschen'						=> $arrConvLoeschen,
							'arr_editieren'						=> $arrConvEditieren,
							'dateinamen'							=> $strDateiNameLoeschen,
							'row'											=> $row,
							'editierlauf'							=> $mkEditierLauf,
						);
					...
					*/
					...
					foreach ($arrEditieren as $strEditierenId)
					{
						...
						(
						 	'dk_beschreibung'			=> $strDokBeschreibung,
							'dk_stichworte'				=> $strDokStichworte,
							'lastedit_member'	=> $intUserId,
							'lastedit_date'		=> $strDatumAktuell,
						);							
						
						$this->Database->prepare("UPDATE `tl_dms_document` %s WHERE id=?")
								 ->set($DsSet)
								 ->execute($strEditierenId);
					...
					*/
					...
					foreach ($rows as $row)
					{
						...
						if (in_array($intId, $arrAltDokVeroeffentlichen))
						{
							...
							(
								'published'	=> 1,
								'lastedit_member' => $mkLasteditUserid,
								'lastedit_date'  => $mkDatum,
							);						
						}
						else
						{
